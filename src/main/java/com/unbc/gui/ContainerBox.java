...
public class ContainerBox {
   ...
   static volatile ContainerBox theContainer;
   
   public static synchronized ContainerBox getInstance()
   {
       if(theContainer == null)
       {
       theContainer = new ContainerBox(0,0,SimulationParameters.WINDOW_SIZE_WIDTH,SimulationParameters.WINDOW_SIZE_HEIGHT);
       }
       return theContainer;
   }
   
   /** Constructors */
   private ContainerBox(int x, int y, int width, int height, Color colorFilled, Color colorBorder) {
      minX = x;
      minY = y;
      maxX = x + width - 1;
      maxY = y + height - 1;
   ...
   private ContainerBox(int x, int y, int width, int height) {
